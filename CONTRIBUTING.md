# Contributing to Kemacs

Kemacs is an early Linux-kernel development environment for Emacs.  Small,
...
Use [GitHub issues](https://github.com/davidlohr/kemacs-mode/issues) for
reproducible bugs and focused design proposals, and
[pull requests](https://github.com/davidlohr/kemacs-mode/pulls) for reviewed
...
This cleans and byte-compiles all `kemacs*.el` and the test file with warnings
...
Kemacs behavior without running the upstream frontend.

## Project boundaries

Keep the dependency direction simple:

1. `kemacs-core.el` contains root discovery, profiles/contexts, capability
...
2. Feature modules require the core and register their own actions.  Runtime
   policy and vng-specific validation belong in `kemacs-debug.el` and
   `kemacs-virtme.el`, not in the generic process layer.
3. `kemacs-mode.el` is the top-level loader/project-mode integration and may
...
Kemacs is an umbrella minor-mode environment, not a replacement C major mode.
The dedicated `kemacs-kconfig-mode` is appropriate because Emacs has no
...
Every external operation must begin with `kemacs-resolve-context`; do not read
...
`kemacs-build-process-environment` so inherited Kbuild selectors cannot
silently replace that context.

Use the generated `kemacs-context-*` accessors.  Do not depend on a struct's
printed representation or the storage layout of `kemacs--selected-profiles`.
...
`kemacs-context-functions` is for quick, deterministic context refinement.
...
Register user-facing operations with `kemacs-register-action` so the current
dispatcher and dashboard can discover them.  Action IDs must be globally
unique symbols.  Choose stable, verb-first command names with the `kemacs-`
...
Pass bare host-tool names to `kemacs-tool-path` only when resolution through
...
add an absolute directory to `kemacs-build-trusted-path-directories` only as a
reviewed escape hatch for an intentional checkout tool shim.  That child-PATH
exception must not silently broaden bare `kemacs-tool-path` resolution.

## External commands

Prefer argument lists throughout the implementation.  Quote only at the final
Compilation-mode boundary with `kemacs-shell-command`, or use a direct process
...
`kemacs-compile`, the mode's managed `compile` remap.
...
automatic network/`sudo` provisioning during runtime; reserve Kemacs-owned
...
`HOME` to `kemacs-vng-home-directory` and inspect exactly its
...
- Use two-space indentation, complete docstrings, and the `kemacs-` namespace.
...
When adding a customization, put it in the `kemacs` group, give it a precise
...
`test/kemacs-test.el`.  Keep tests independent of personal Emacs setup and
